...
_unit_tests() {
  ...
  go run github.com/onsi/ginkgo/v2/ginkgo --tags testutils --repeat 1 -r --output-dir ${_test_report_dir} --json-report unit_tests.json $* ./... > ${_test_report_dir}/unit_tests.log 2>&1
  local _result=$?
